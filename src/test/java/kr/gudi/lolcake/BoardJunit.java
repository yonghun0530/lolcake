...
public class BoardJunit {
	...
	/*
	 * @Test public void BoardTest() throws Exception {
	 * 
	 * mock.perform(get(URI) // get방식 : get("주소"), post방식 : post("주소")
	 * .param("start", "0") // paramater값 설정 : .param("key", "value")
	 * .param("viewRow", "10") .param("type", type)) .andDo(new ResultHandler() {//
	 * 처리 내용을 출력합니다.
	 * 
	 * @Override public void handle(MvcResult arg0) throws Exception { ModelAndView
	 * mav = arg0.getModelAndView(); Map<String, Object> map = mav.getModel();
	 * System.out.println(map);
	 * 
	 * String message = map.get("message").toString(); System.out.println(message);
	 * JsonParser parser = new JsonParser(); JsonElement element =
	 * parser.parse(message);
	 * 
	 * JsonObject jobject = element.getAsJsonObject(); JsonArray list =
	 * jobject.get("list").getAsJsonArray(); System.out.println(list);
	 ...
	 * .andExpect(model().attributeExists("message"));// "message"이라는 attribute가
	 * 존재해야 합니다.
	 * 
	 * }
	 */
	...
	public void TestCommnetWrite() throws Exception {
		mock.perform(get("/replyWrite") // get방식 : get("주소"), post방식 : post("주소")
				.param("boardno", bno).param("comment", cmt).param("id", id).param("passwd", passwd)) // paramater값 설정 :
																										// .param("key",
																										// "value")
				.andDo(new ResultHandler() {// 처리 내용을 출력합니다.
					@Override
	...
	public void TestCommnetEdit() throws Exception {
		mock.perform(get("/reEdit") // get방식 : get("주소"), post방식 : post("주소")
				.param("no", no).param("comment", cmt)) // paramater값 설정 : .param("key", "value")
				.andDo(new ResultHandler() {// 처리 내용을 출력합니다.
					@Override
					public void handle(MvcResult arg0) throws Exception {
						ModelAndView mav = arg0.getModelAndView();
